...
describe('BinaryExpressionTests', () => {
  ...
  it('incompleteAddition', async () => {
    let expression = parseExpressionExpectException(`   Result = Value +`, "(BinaryExpression) No tokens right from: 1 (StringLiteralToken('Value') OperatorToken('+') )");
  });
});
