...
function renderBinaryExpression(expression: BinaryExpression, codeWriter: CodeWriter) {
  const expressionState = expression.stateRequired;
  if (expressionState.leftType?.equals(ValueType.number) && expressionState.rightType?.equals(ValueType.number)) {
    if (renderNumberOperationExpression(expression, codeWriter)) {
      return;
    }
  }
  renderExpression(expression.left, codeWriter);
  if (expressionState.leftType?.equals(ValueType.date)) {
    codeWriter.write(".getTime()");
  }

  codeWriter.write(operatorString(expression.operator));

  if (expressionState.leftType?.equals(ValueType.string) &&  expressionState.rightType?.equals(ValueType.date)) {
