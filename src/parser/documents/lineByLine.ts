...
//adaption from https://github.com/nacholibre/node-readlines/
type Options = {
...
export class LineByLine {
  ...
  private readonly isStdin: boolean;
  private readonly options: Options;
  private fd: number | null;
  private eofReached = false;
  private linesCache: Buffer[] = [];
  private fdPosition = 0;
  private lastChunkEndedWithCR = false;
  private lineIsComplete = false;

  constructor(file: string, options: Options | null = null) {
  ...
  private static searchInBuffer(buffer: Buffer) {
  ...
  private readChunk(lineLeftovers: Buffer | null) {
  ...
  private extractLines(buffer: Buffer, isEof: boolean): Buffer[] {
    const lines = [];
    let lineStart = 0;
