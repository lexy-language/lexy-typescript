...
export function instanceOfDateTimeLiteral(object: any): boolean {
  return object?.tokenType == TokenType.DateTimeLiteralToken;
}

export function asDateTimeLiteral(object: any): DateTimeLiteralToken | null {
  return instanceOfDateTimeLiteral(object) ? object as DateTimeLiteralToken : null;
}

const charT = 'T'.charCodeAt(0);

export class DateTimeLiteralToken extends ParsableToken implements ILiteralToken {
  ...
  private static readonly SeparatorIndexes = [10];
  ...
  public tokenType = TokenType.DateTimeLiteralToken;

  constructor(character: TokenCharacter) {
    super(character, '');
    this.validators = [
      value => this.validate(value, isDigit(value), DateTimeLiteralToken.DigitIndexes),
      value => this.validateExactMultiple(value, [TokenValues.Dash, TokenValues.Slash], DateTimeLiteralToken.DashIndexes),
      value => this.validateExact(value, TokenValues.Colon, DateTimeLiteralToken.ColonIndexes),
      value => this.validateExactMultiple(value, [charT, TokenValues.Space], DateTimeLiteralToken.SeparatorIndexes)
    ];
  }
    ...
    if (value == TokenValues.Quote) {
      if (DateTimeLiteralToken.ValidLengths.findIndex(item => item == this.value.length) < 0) {
        return newParseTokenInvalidResult(
          ...
          Expected: '${formatLine(DateTimeLiteralToken.ValidLengths, ',')}'`);
      }
  ...
  private validateExactMultiple(value: number, matches: Array<number>, indexes: number[]): ParseTokenResult | null {
